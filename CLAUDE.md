...
11. **Usar la Infraestructura Karpathy.** Al iniciar, consultar `AGENTS.md` y la ficha central de SmartWaste cuando esté disponible. Al cerrar una tarea con conocimiento duradero, usar `docs/KARPATHY_TASK_HANDOFF.md` para proponer una actualización trazable de `MTIT-Blueprint`. El código y la evidencia local prevalecen si la síntesis central está desactualizada; nunca copiar secretos, datos municipales ni registros crudos a la infraestructura.
