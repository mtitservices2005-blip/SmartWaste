...
11. **Usar la Infraestructura Karpathy.** Al iniciar, consultar `AGENTS.md` y `docs/KARPATHY_CONTEXT.md`; si `MTIT-Blueprint` está accesible, comprobar también la ficha central más reciente. La falta de acceso central no bloquea el trabajo seguro: registrar la revisión de la copia local y continuar. Al cerrar una tarea con conocimiento duradero, usar `docs/KARPATHY_TASK_HANDOFF.md` para proponer una actualización trazable. El código y la evidencia local prevalecen si una síntesis está desactualizada; nunca copiar secretos, datos municipales ni registros crudos a la infraestructura.
