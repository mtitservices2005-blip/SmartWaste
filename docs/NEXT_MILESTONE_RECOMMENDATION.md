...
4. Sembrar datos de prueba mínimos (un municipio, 2-3 perfiles con roles distintos incluido `driver`, algunas rutas).
5. Ejecutar pruebas RLS adversariales reales: intentar leer/escribir cruzando `municipality_id` con cada rol y confirmar que Postgres deniega lo que debe denegar. Esto reemplaza — no complementa — el matching de texto de `tests/rls-static.test.mjs`.
6. Conectar `createSupabaseOperationsAdapter` y `resolveSupabaseAuthContext` a un entorno de prueba real (puede ser un script/test de Node, no necesariamente el frontend todavía) y ejercer al menos un ciclo completo: crear ruta → asignar → iniciar → progreso → completar → verificar, contra la base local.
7. Re-clasificar en `docs/CURRENT_STATE_AUDIT.md` cada ítem que pase la verificación a `VERIFIED_REAL`, con evidencia (comando ejecutado, resultado).
...
Quedan documentadas como trabajo futuro (ver `docs/TECHNICAL_DEBT_REGISTER.md` #5 y #11), pero explícitamente después de SW-020, no antes ni en paralelo.
